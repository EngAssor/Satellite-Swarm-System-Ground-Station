...
def start_session():
    ...
    while command_counter>0:
        ...
        if obc_on == 1:
            ...
            progressinsert(msg)
            TLM.insert(INSERT, 'Received image Successfully ' + '\n')
            first_window.update()
            time.sleep(2)
            #print('beforeeee')
            r = requests.get('http://192.168.1.50:2560/')
            #print('beforeeee1')
            comm = r.content
            #print('beforeeee2')
...
mass_boxx("Welocme",'The Program Wating For Connection To Be Established ')
stablish()
